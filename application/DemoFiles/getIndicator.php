<h1> Search Indicators by Site or Year</h1>
...
if (isset($_POST['submit'])) {
    ...
    $year = $_POST['year'];
      ...
      $query = "SELECT * FROM air_quality_indicator WHERE local_site_name = :local_site_name OR year = :year" ;
    ...
    <table>
      <thead>
		<tr>
          <th>Site</th>
          <th>Year</th>
          <th>Number of observation below MDL</th>
...
<?php foreach ($result as $row) { ?>
      ...
      <tr>
        ...
        <td><?php echo $row["num_obs_below_mdl"]; ?></td>
        <td><?php echo $row["arithmetic_mean"]; ?></td>
        <td><?php echo $row["arithmetic_standard_dev"]; ?></td>
        <td><?php echo $row["primary_exceedance_count"]; ?></td>
        <td><?php echo $row["secondary_exceedance_count"]; ?></td>
      </tr>
<?php } ?>
      </tbody>
  </table>
